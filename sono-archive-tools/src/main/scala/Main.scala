...
object Main {
  ...
  private def mongoSample(): Unit = {
    ...
    val tagsToFilter = Set(
      new AttributeTag(0x0008, 0x0012),
      new AttributeTag(0x0010, 0x0020)
    )
    ...
    filteredDicomMeta.foreach { case (key, value) =>
      ...
      val zoned = if (isValidDateFormat(matches(2))) convertToZonedDateTime(matches(2)) else ""
  ...
  private def convertToZonedDateTime(dateString: String): ZonedDateTime = {
